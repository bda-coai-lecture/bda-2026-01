...
| ID | 계층 | 시나리오 | 절차 | 기대 결과 |
|---|---|---|---|---|
| BOT-P0-01 | L | 답변 형식 canary | 새 Slack 스레드에서 1일 mart로 답할 수 있는 짧은 질문을 멘션한다 | 첫 문장이 결론이다. “아래가 답변입니다”, “분석 완료”, “모드: 활동량”, “인풋”, “관측 grain”, dry run, `bq query`, `dbt compile`, `Claude N턴`, `BigQuery N GiB` 같은 내부/운영 라벨과 `---`가 없다. 본문은 약 2,500자 이내이고 Slack mrkdwn만 사용한다 |
| BOT-P0-02 | L | PO용 푸터 분리 | BOT-P0-01의 최종 답변 하단을 확인한다 | 답변 메시지 context footer에는 실행 시간과 “상세 실행 내역은 로그에 저장됨”만 보인다. Claude 환산 비용, BigQuery GiB/USD/job 수, dry-run 바이트, 검증/재현 SQL 목록은 Slack에 없고 audit/trace JSONL에만 남는다 |
| BOT-P0-03 | L | 진행 메시지 정리 | 2개 이상의 도구를 쓰는 질문을 실행한다 | 실행 중에는 사람이 읽는 진행 상황과 `N초 경과 / 최대 N분`만 갱신되고 raw tool trace·dry-run 문구가 보이지 않는다. 성공 시 같은 메시지가 최종 답변으로 교체되고, 실패/중단 시 해당 상태로 접힌다 |
...
| BOT-P0-11 | L | Metabase 카드 선택 생성 | `ANALYST_ENABLE_METABASE_MCP=1`, `METABASE_URL`, `METABASE_API_KEY`를 설정하고 추이/Top-N 질문을 보낸다 | 필요할 때 Metabase 카드 또는 대시보드가 생성되고 Slack에는 클릭 가능한 URL/버튼만 보인다. MCP/tool/collection id/API 세부값은 보이지 않는다 |
...
| BOT-U-18 | 푸터 계산 | 0 job / 여러 job / null Claude cost | Slack 푸터에는 PO용 문구만 표시하고, audit에는 실제 billed bytes 합을 보존 |
| BOT-U-19 | Metabase MCP config | enable off/on, URL/API key 있음 | off에서는 MCP 도구·config가 없고, on에서는 `--strict-mcp-config --mcp-config`가 생긴다. API key는 argv/Slack/audit에 노출되지 않고 credential 파일명은 Read 차단 대상이다 |
| BOT-U-20 | Metabase 링크 버튼 | 최종 답변에 `question/{id}` 또는 `dashboard/{id}` URL 포함 | Slack block에 `Metabase 열기` 버튼이 추가됨 |

## 4. C — 외부 서비스 없는 모의 통합

| ID | 시나리오 | 주입할 fake | 기대 결과 |
|---|---|---|---|
| BOT-C-01 | 정상 1턴 | Claude 성공 stream, Slack client, BQ usage 2건 | 진행 메시지 → 같은 메시지가 최종 답변으로 교체. Slack footer는 PO용이고, 비용/scan 세부값은 audit에만 기록 |
...
| BOT-C-04 | Claude non-zero | stderr와 permission denial | 사용자용 실패 안내만 보이고 raw stderr/JSON은 Slack에 나오지 않음. 성공 답변 없음 |
| BOT-C-05 | Slack `chat.update` 실패 | update만 예외, post는 성공 | 분석 결과는 새 메시지로 전달되고 러너가 죽지 않음 |
| BOT-C-06 | Slack final delivery 실패 | 최종 `chat.update`와 fallback post 모두 예외 | audit status가 `delivery_error`이고 success로 기록하지 않는다. 무한 재시도 없음 |
| BOT-C-07 | 중복 이벤트 | 같은 Slack event_id 두 번 | 분석 턴과 답변이 한 번만 생성됨 |
| BOT-C-08 | 멘션 없는 후속 답글 | thread reply without app mention | 무시하며 분석 작업을 시작하지 않음 |
...
| BOT-C-12 | BQ 집계 권한 실패 | operator usage query 예외 | 분석 답변은 보존하고 audit에 집계 실패를 기록. Slack에는 raw 오류·집계 세부값을 노출하지 않음 |
...
| BOT-L-01 | mart-only 짧은 질문 | raw 미조회, 간결한 답변. dry-run·검증 상세·재현 SQL 경로는 로그/audit에만 남음 |
| BOT-L-02 | raw 필요 질문 | `_TABLE_SUFFIX` 범위와 dry run은 로그/audit에서 확인되고 Slack에는 결론·기준·한계만 보임 |
| BOT-L-03 | 계측 공백 질문 | 숫자를 꾸며내지 않고 `계측 공백`과 필요한 추가 수집을 제시 |
| BOT-L-04 | 후속 질문 | 같은 스레드에서 앞선 정의와 기간을 기억하되, 지표 변경 시 변경 사실 명시 |
| BOT-L-05 | Metabase 선택 시각화 | 시계열/Top-N 질문 + MCP 활성 | 카드/대시보드 생성, Slack URL/버튼 노출, 불필요한 단일 숫자 질문에는 카드 생성하지 않음 |
...
첫 자동화 묶음은 외부 과금 없이 자주 돌릴 수 있는 다음 14개다.

`BOT-U-01`, `BOT-U-02`, `BOT-U-04`, `BOT-U-05`, `BOT-U-09`, `BOT-U-10`,
`BOT-U-12`, `BOT-U-13`, `BOT-U-17`, `BOT-U-19`, `BOT-U-20`, `BOT-C-01`, `BOT-C-03`, `BOT-C-10`.
