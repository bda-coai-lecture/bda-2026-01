...
- 2026-05-16까지 최신화한 mart/feature 기준에서도 LGBM re-rank가 가장 안정적이었다.
- 같은 candidate set re-rank 비교에서는 NDCG@100 기준 `LGBM > FM/Deep&Wide/DeepFM/DLRM > ALS/Fallback > Popularity` 흐름이 유지됐다. 다만 최신 5/16 run의 neural ranker들은 같은 순위로 수렴해, 모델 우열보다 학습/튜닝 한계로 해석하는 것이 맞다.
...
Airflow가 생성한 최신 recommendation mart와 ranker feature parquet를 사용했다.

실험 설정:

- history: 2026-03-22 ~ 2026-05-02
- rank: 2026-05-03 ~ 2026-05-09
- test: 2026-05-10 ~ 2026-05-16
- `max_items=100000`
- `candidate_k=120`, `hybrid_extra=80`
- `related_candidate_cap=30`, `related_top_per_anchor=10`, `related_max_seen_anchors=10`
- ALS `factors=48`, `iterations=4`
...
| 항목 | 값 |
|---|---:|
| history interactions | 2,377,667 |
| history users | 1,217,036 |
| history repos | 100,000 |
| rank labels | 361,371 |
| test labels | 331,702 |
| rank rows | 163,400 |
| positive labels | 877 |

### Same Candidate Re-rank

| 모델 | NDCG@10 | NDCG@50 | NDCG@100 | Recall@100 | Unique@100 |
|---|---:|---:|---:|---:|---:|
| Popularity | 0.004185 | 0.009907 | 0.014227 | 0.064901 | 105 |
| ALS/Fallback | 0.008321 | 0.015396 | 0.018460 | 0.065736 | 6,644 |
| Two-Stage/LGBM | 0.010989 | 0.019443 | 0.022160 | 0.075824 | 4,983 |
| FM | 0.006771 | 0.012726 | 0.015860 | 0.057748 | 6,838 |
| Deep&Wide | 0.006771 | 0.012726 | 0.015860 | 0.057748 | 6,838 |
| DeepFM | 0.006771 | 0.012726 | 0.015860 | 0.057748 | 6,838 |
| DLRM | 0.006771 | 0.012726 | 0.015860 | 0.057748 | 6,838 |

해석:

- 최신 데이터에서도 LGBM re-rank가 NDCG@10/100과 Recall@100 모두 가장 높다.
- neural ranker들은 ALS보다는 일부 cut-off에서 낫지만 LGBM에는 못 미친다.
- FM/Deep&Wide/DeepFM/DLRM 수치가 같은 것은 모델이 충분히 분화했다기보다, 163k rank rows와 877 positive label에서 BCE ranker가 비슷한 후보 순서로 수렴한 결과로 보는 편이 안전하다.
- 현재 강의 결론은 "retrieval 교체보다 re-rank 개선이 더 확실하고, tabular LGBM은 여전히 강한 기준선"이다.

산출물:

- `data/models/week6/week6_ranker_compare_airflow_20260516_neural_compare_metrics.csv`
- `data/models/week6/week6_ranker_compare_airflow_20260516_neural_compare_summary.json`
- `data/models/week6/lgbm_ranker_compare_airflow_20260516_neural_compare.txt`
- `data/models/week6/fm_ranker_airflow_20260516_neural_compare.pt`
- `data/models/week6/deepwide_ranker_airflow_20260516_neural_compare.pt`
- `data/models/week6/deepfm_ranker_airflow_20260516_neural_compare.pt`
- `data/models/week6/dlrm_ranker_airflow_20260516_neural_compare.pt`

### Two-Tower Retrieval

Two-Tower는 `airflow_20260516_lgbm_eval`의 ALS/mapping과 같은 100k-item catalog를 사용했다. 이 평가는 warm user 중심 retrieval 비교라 위 re-rank 표와 직접 비교하지 않는다.

| 모델 | NDCG@10 | NDCG@50 | NDCG@100 | Recall@100 | Unique@100 |
|---|---:|---:|---:|---:|---:|
| Popularity | 0.002403 | 0.006547 | 0.009140 | 0.040955 | 106 |
| ALS | 0.009135 | 0.011912 | 0.013327 | 0.036928 | 7,136 |
| Two-Tower | 0.003513 | 0.005231 | 0.006745 | 0.022170 | 70,537 |

해석:

- Two-Tower는 ALS보다 정확도는 낮지만 추천 다양성은 훨씬 크다.
- 현재 상태에서는 ALS 대체재가 아니라 후보 다양성 보강 source로 보는 것이 맞다.
- 제대로 경쟁시키려면 negative sampling, loss, epoch, item/user feature schema, ANN recall까지 별도 튜닝이 필요하다.

산출물:

- `data/models/week6/two_tower_airflow_20260516_two_tower_metrics.csv`
...
### 2026-07-05 전체 warm user 재학습 확인

강의 정리 전 최신 mart 경로가 여전히 정상 동작하는지 확인하기 위해 `airflow_20260516_lgbm_eval` ALS/mapping을 재사용하고 Two-Tower를 3 epoch로 다시 학습했다. item catalog는 저장된 ALS/mapping의 100k repo를 사용했고, 평가는 test split의 warm user 전체 141,190명을 사용했다.
...
해석은 기존 결론과 같다. 전체 warm user 평가에서도 ALS가 정확도 기준으로 앞서고, Two-Tower는 추천 catalog 다양성이 훨씬 크다.

산출물:

- `data/models/week6/two_tower_latest_20260705_full_e3_metrics.csv`
- `data/models/week6/two_tower_latest_20260705_full_e3_summary.json`
- `data/models/week6/two_tower_latest_20260705_full_e3.pt`
...
MLflow 실행/실험명/run id/비교 컬럼은 `docs/mlflow_recsys_tracking.md`에서 별도로 관리한다.
