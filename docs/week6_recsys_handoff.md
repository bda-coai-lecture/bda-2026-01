...
- 데이터: `data/daily_agg`, 현재 로컬 보유 범위는 `2026-01-01` ~ `2026-05-16`이다. Week 6 추천 실험 window는 아래 history/rank/test 기간을 그대로 사용한다.
- 최신 자동화 split:
...
- 최신 5/16 smoke-scale 검증: `airflow_20260516_lgbm_eval`, `Two-Stage/Fallback NDCG@10 = 0.010674`, `Recall@100 = 0.075490`
...
- Airflow DAG `dags/gharchive_recsys_features.py`가 최신 mart/feature 생성을 자동화한다. Docker 메모리 한계 때문에 최신 산출물이 있으면 skip하는 cache-aware 방식이다.
...
### 최신 2026-05-16 자동화 산출물 검증

Airflow가 생성한 mart와 ranker feature parquet를 사용해 최신 데이터 기준으로 재학습/평가했다.

입력 산출물:

- `data/marts/week6/user_repo_interaction_mart.parquet`
- `data/marts/week6/experiment_split_mart.parquet`
- `data/marts/week6/repo_feature_mart.parquet`
- `data/marts/week6/user_profile_mart.parquet`
- `data/marts/week6/repo_repo_related_mart.parquet`
- `data/features/week6/ranker_features_airflow_20260516.parquet`
- `data/features/week6/ranker_features_airflow_20260516_summary.json`
...
- ALS `factors=48`, `iterations=4`, `regularization=0.03`
- `eval_users=3000`
- MLflow run: `b4e1a76a92ed46608ca747b3d7482d31`

결과:

| model | k | precision | recall | ndcg | unique_recommended |
|---|---:|---:|---:|---:|---:|
| Popularity | 10 | 0.001400 | 0.012614 | 0.004185 | 12 |
| Popularity | 100 | 0.000767 | 0.064901 | 0.014227 | 105 |
| ALS/Fallback | 10 | 0.001900 | 0.016167 | 0.008158 | 1,197 |
| ALS/Fallback | 100 | 0.000770 | 0.066069 | 0.018430 | 6,644 |
| Two-Stage/Fallback | 10 | 0.002733 | 0.022691 | 0.010674 | 820 |
| Two-Stage/Fallback | 100 | 0.000900 | 0.075490 | 0.021894 | 5,200 |

해석:

- 최신 데이터에서도 `Popularity < ALS < Two-Stage` 구조가 유지된다.
- `Recall@100` 기준 Two-Stage는 ALS/Fallback 대비 `0.066069 -> 0.075490`으로 개선된다.
- 이 run은 운영 자동화 검증용 100k catalog / 3k eval 설정이다. 기존 최고 full run과 직접 수치 비교하지 않는다.
- MLflow 실행/확인 방법은 `docs/mlflow_recsys_tracking.md`를 본다.
