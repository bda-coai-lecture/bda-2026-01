...
Week 7 데이터 핸들링용 dbt project다. BigQuery의 `mart.fact_user_repo_activity`를 source로 보고, dashboard용 metric mart와 dbt Semantic Layer 정의를 만든다.
...
Airflow에서는 `dags/gharchive_dbt_metrics.py`가 local parquet를 `fact_user_repo_activity`에 먼저 적재한 뒤, Cosmos `DbtTaskGroup`으로 dbt model/test를 실행한다.
